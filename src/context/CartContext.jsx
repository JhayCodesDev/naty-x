...
export function CartProvider({ children }) {
  ...
  function updateVariant(key, { size, color }) {
    setCartItems((prev) =>
      prev.map((item) => {
        ...
        const newKey = cartLineKey(item.id, size ?? item.size, color ?? item.color)
        return { ...item, size: size ?? item.size, color: color ?? item.color, key: newKey }
      })
    )
  }
  ...
  const cartSubtotal = useMemo(() => cartItems.reduce((sum, i) => sum + i.price * i.quantity, 0), [cartItems])
